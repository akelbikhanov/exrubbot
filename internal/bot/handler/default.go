...
func (h *Handler) DefaultHandler() bot.HandlerFunc {
	return func(ctx context.Context, b *bot.Bot, update *models.Update) {
		...
		defer func() {
			...
			logger.Info(fmt.Sprintf(text.InfoUpdateProcessingTime, update.ID, elapsed))
		}()

		switch {
		case update.Message != nil:
			h.messageHandler(ctx, b, update.Message)
		case update.CallbackQuery != nil:
			h.callbackHandler(ctx, b, update.CallbackQuery)
		default:
			logger.Info(fmt.Sprintf(text.InfoUpdateProcessingSkip, getUpdateType(update)))
		}
	}
}
